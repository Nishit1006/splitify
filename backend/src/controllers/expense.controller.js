...
export const addExpense = asyncHandler(async (req, res) => {
    ...
    // ✅ VALIDATE SPLITS BEFORE CREATING ANYTHING IN DB
    const type = splitType || "EQUAL";
    let expenseSplits = [];

    if (type === "EQUAL") {
        if (splits.length === 0) {
            throw new ApiError(400, "Splits array cannot be empty");
        }
        const perPersonAmount = totalAmount / splits.length;
        expenseSplits = splits.map(userId => ({
            userId,
        ...
        if (Math.abs(totalSplit - totalAmount) > 0.01) {
            throw new ApiError(
                400,
                `Split amounts (${totalSplit.toFixed(2)}) don't match total amount (${totalAmount})`
            );
        }
        expenseSplits = splits.map(s => ({
            userId: s.userId,
        ...
        if (Math.abs(totalPercentage - 100) > 0.01) {
            throw new ApiError(
                400,
                `Percentages must add up to 100. Current total: ${totalPercentage.toFixed(2)}%`
            );
        }
        expenseSplits = splits.map(s => ({
            userId: s.userId,
    ...
    // ✅ ALL VALIDATIONS PASSED — NOW CREATE IN DB
    const expense = await Expense.create({
        groupId,
        title: title.trim(),
        description: description?.trim(),
    ...
    // Attach expenseId to each split now that expense is created
...
export const updateExpense = asyncHandler(async (req, res) => {
    ...
    // ✅ VALIDATE NEW SPLITS BEFORE UPDATING ANYTHING IN DB
    if (splits && Array.isArray(splits) && splits.length > 0) {
        ...
        if (newType === "EQUAL") {
            const perPersonAmount = newAmount / splits.length;
            validatedSplits = splits.map(userId => ({
                userId,
                splitValue: 1,
                finalAmount: perPersonAmount
            }));
            ...
            if (Math.abs(totalSplit - newAmount) > 0.01) {
                throw new ApiError(
                    400,
                    `Split amounts (${totalSplit.toFixed(2)}) don't match total amount (${newAmount})`
                );
            }
            validatedSplits = splits.map(s => ({
                userId: s.userId,
                splitValue: s.amount,
                finalAmount: s.amount
            }));
            ...
            validatedSplits = splits.map(s => ({
                userId: s.userId,
                splitValue: s.percentage,
                finalAmount: (newAmount * s.percentage) / 100
            }));
        ...
        // ✅ VALIDATION PASSED — NOW UPDATE EXPENSE AND SPLITS
        if (title) expense.title = title.trim();
        if (description !== undefined) expense.description = description?.trim();
        if (totalAmount) expense.totalAmount = totalAmount;
        if (splitType) expense.splitType = splitType;
        ...
        const splitsWithExpenseId = validatedSplits.map(s => ({
            ...s,
            expenseId: expense._id
        }));
        ...
        // No splits update — just update expense fields
        if (title) expense.title = title.trim();
        if (description !== undefined) expense.description = description?.trim();
        if (totalAmount) expense.totalAmount = totalAmount;
        if (splitType) expense.splitType = splitType;
